 ...
 * @Date: 2023-12-19 09:14:43
 * @LastEditors: zhengjiefeng zhengjiefeng
 * @LastEditTime: 2023-12-19 16:00:12
 * @FilePath: \vite-vue3-temp\src\views\markCrossWindow\Index.vue
 * @Description: 
 * 
-->
<template>
  <div class="container">
    ...
    <br />
    <div class="alert" ref="alertRef"></div>
  </div>
</template>
...
const alertRef = ref()
let { camera, renderer, scene } = win.init()
let key = []
let balls = []
...
function init() {
  // console.log(win.init(), '111111111111111')
...
function animate() {
  ...
  balls.forEach(({ ball, key: k, win }) => {
    ...
    balls
      ...
      .forEach(({ ball: ball2, key: k2, win: win2 }) => {
        const moveP = {
...
function log() {
  ...
  alertRef.value.textContent = JSON.stringify(
    {
      key,
      screenX,
      screenY,
