...
const OUTER_RADIUS = 150,
  INNER_RADIUS = 80 // 大小球体的半径
const OUTER_SPRITE_WIDTH = 10,
  INNER_RSPRITE_WIDTH = 5 // 大小球体上的粒子高宽
const OUTER_SPRITE_COUNT = 300,
  INNER_RSPRITE_COUNT = 50,
  MOVE_SPRITE_COUNT = 200 // 大小球体、及连线上的粒子个数
...
const materialTemp = (function () {
  ...
  const map = textureLoader.load(sprite)
...
export default class BALL {
  ...
  renderMove(movePs) {
    ...
    movePs.forEach(({ key, moveP }) => {
      ...
      if (
        !moveP ||
        (Math.abs(moveP.x) <= OUTER_RADIUS * 2 && Math.abs(moveP.y) <= OUTER_RADIUS * 2)
      ) {
        if (move.moving == true) {
          move.moving = false
          move.frame = 0
      ...
      move.frame++
      if (move.frame % MOVE_FRAME_GAP === 0) {
        // 每10帧开始移动一个粒子，达到粒子先后依次移动射线效果
        move.index++
      }
      move.groups.forEach((group, i) => {
        ...
        if (group.index == MOVE_FRAME_NUM) {
          // 粒子移动具体帧数后，重置位置，重新移动
          group.visible = false
          group.index = 0
          this.clearGroupPosition(group)
        } else if (gapX < 0.5 || gapY < 0.5) {
          // 粒子移动临近目的位置时，隐藏，等待帧数结束重新移动
          group.index++
          group.visible = false
        } else {
          group.index++
          ...
          const radius =
            OUTER_RADIUS - (group.index * (OUTER_RADIUS - INNER_RADIUS / 2)) / MOVE_FRAME_NUM
          const width =
            OUTER_SPRITE_WIDTH -
            (group.index * (OUTER_SPRITE_WIDTH - INNER_RSPRITE_WIDTH)) / MOVE_FRAME_NUM
